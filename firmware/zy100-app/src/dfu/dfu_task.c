...
void dfu_main_task(void *p_param)
{
    ...
    dfu_timer_init();
    zy100_ota_led_start_blue_breathing();

    while (true)
    {
        if (dfu_service_poll_transfer_timeout()) return;
        dfu_watchdog_poll((uint32_t)os_sys_time_get());
        received = os_msg_recv(dfu_evt_queue_handle, &event,
                              dfu_service_transfer_wait_ms(dfu_watchdog_task_wait_ms()));
        /* A queued event arriving at the deadline must not revive transfer. */
        if (dfu_service_poll_transfer_timeout()) return;
        if (received)
        {
