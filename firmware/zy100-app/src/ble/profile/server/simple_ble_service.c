...
#define ZY100_LOG_PROFILE_INFO_ENABLE 0
#endif

#if !ZY100_LOG_PROFILE_INFO_ENABLE
...
#define SIMPLE_BLE_SERVICE_CHAR_V5_CFG_INDEX            0x0d
...
#define SIMPLE_BLE_SERVICE_CHAR_V5_CFG_CCCD_INDEX       (SIMPLE_BLE_SERVICE_CHAR_V5_CFG_INDEX + 1)
...
static uint8_t simple_char_v5_cfg_value[SIMP_READ_V5_MAX_LEN];
static uint16_t simple_char_v5_cfg_len = SIMP_READ_V5_MAX_LEN;
...
const T_ATTRIB_APPL simple_ble_service_tbl[] =
{
    ...
    /* <<Characteristic>>, demo for v5 sensor cfg */
    {
        ATTRIB_FLAG_VALUE_INCL,                     /* flags */
        {                                           /* type_value */
            LO_WORD(GATT_UUID_CHARACTERISTIC),
    ...
    },
    {
        ATTRIB_FLAG_VALUE_APPL,                     /* flags */
        {                                           /* type_value */
            LO_WORD(GATT_UUID_CHAR_SIMPLE_V5_CFG),
            HI_WORD(GATT_UUID_CHAR_SIMPLE_V5_CFG)
        },
        0,                                          /* bValueLen */
        NULL,
#if SIMP_SRV_AUTHEN_EN
...
bool simp_ble_service_set_parameter(T_SIMP_PARAM_TYPE param_type, uint16_t len, void *p_value)
{
    ...
    case SIMPLE_BLE_SERVICE_PARAM_V1_READ_CHAR_VAL:
        ...
        break;
    case SIMPLE_BLE_SERVICE_PARAM_V5_CFG_CHAR_VAL:
        if (len == SIMP_READ_V5_MAX_LEN)
        {
            memcpy(simple_char_v5_cfg_value, p_value, len);
            simple_char_v5_cfg_len = len;
        }
        else
        {
            ret = false;
                                            ...
                                            uint16_t attrib_index, uint16_t offset, uint16_t *p_length, uint8_t **pp_value)
{
    ...
    case SIMPLE_BLE_SERVICE_CHAR_V1_READ_INDEX:
        ...
        break;
    case SIMPLE_BLE_SERVICE_CHAR_V5_CFG_INDEX:
        {
            TSIMP_CALLBACK_DATA callback_data;
            callback_data.msg_type = SERVICE_CALLBACK_TYPE_READ_CHAR_VALUE;
            callback_data.msg_data.read_value_index = SIMP_READ_V5;
            callback_data.conn_id = conn_id;
            if (pfn_simp_ble_service_cb)
            {
                pfn_simp_ble_service_cb(service_id, (void *)&callback_data);
            }
            *pp_value = simple_char_v5_cfg_value;
            *p_length = simple_char_v5_cfg_len;
        }
        break;
                                            ...
                                            P_FUN_WRITE_IND_POST_PROC *p_write_ind_post_proc)
{
    ...
    *p_write_ind_post_proc = simple_write_post_callback;
    if (SIMPLE_BLE_SERVICE_CHAR_V2_WRITE_INDEX == attrib_index)
    {
        ...
        if ((p_value == NULL) || (length > SIMP_WRITE_V2_MAX_LEN))
        {
            APP_PRINT_ERROR2("simp_ble_service_attr_write_cb invalid write: length %d, max %d",
                             length, SIMP_WRITE_V2_MAX_LEN);
    ...
    else if (SIMPLE_BLE_SERVICE_CHAR_V5_CFG_INDEX == attrib_index)
    {
        if ((p_value == NULL) || (length > SIMP_WRITE_V5_MAX_LEN))
        {
            APP_PRINT_ERROR2("simp_ble_service_attr_write_cb invalid v5 write: length %d, max %d",
                             length, SIMP_WRITE_V5_MAX_LEN);
            cause = APP_RESULT_INVALID_VALUE_SIZE;
        }
        else
        {
            callback_data.msg_type = SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE;
            callback_data.conn_id  = conn_id;
            callback_data.msg_data.write.opcode = SIMP_WRITE_V5;
            callback_data.msg_data.write.write_type = write_type;
            callback_data.msg_data.write.len = length;
            callback_data.msg_data.write.p_value = p_value;
...
/**
  * @brief send notification of simple v5 cfg characteristic value.
  */
bool simp_ble_service_send_v5_notify(uint8_t conn_id, T_SERVER_ID service_id, void *p_value,
                                     uint16_t length)
{
    return server_send_data(conn_id, service_id, SIMPLE_BLE_SERVICE_CHAR_V5_CFG_INDEX, p_value,
                            length, GATT_PDU_TYPE_ANY);
}
                                     ...
                                     uint16_t cccbits)
{
    ...
    case SIMPLE_BLE_SERVICE_CHAR_INDICATE_CCCD_INDEX:
        ...
        break;
    case SIMPLE_BLE_SERVICE_CHAR_V5_CFG_CCCD_INDEX:
        {
            if (cccbits & GATT_CLIENT_CHAR_CONFIG_NOTIFY)
            {
