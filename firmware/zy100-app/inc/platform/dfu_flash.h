...
/** @defgroup DFU_API DFU API Sets
  ...
  * @{
  */
#define EXTERNAL_SPI_FLASH_START_ADDR                   0x02800000

#define IS_EXTERNAL_SPI_FLASH_ADDR(addr)                (((addr & ~FLASH_OFFSET_TO_NO_CACHE) >= EXTERNAL_SPI_FLASH_START_ADDR) ? true : false)
#define EXTERNAL_SPI_FLASH_ADDR_TO_OFFSET(addr)         ((addr & ~FLASH_OFFSET_TO_NO_CACHE) - EXTERNAL_SPI_FLASH_START_ADDR)
...
/* Compatibility aliases for the product DFU overlay retained from the
 * previous SDK. The v1.1.0 SDK exposes the same lookup through the names
 * below, without the historical _app suffix. */
...
{
    return get_temp_ota_bank_size_by_img_id(image_id);
}
 ...
 * @return 0 if report FW info successfully, error line number otherwise
*/
uint32_t dfu_report_target_fw_info_app(uint16_t image_id, uint32_t *p_origin_fw_version,
                                       uint32_t *p_offset);
#endif
...
/**
 * @brief Validate the temporary image without changing any ready bits.
 * @param image_id image id to identify image.
 * @return true if the temporary image identity and checksum are valid.
 */
bool dfu_validate_checksum_only(uint16_t image_id);

/**
 * @brief Revalidate the temporary image, commit its ready bit, and verify readback.
 * @param image_id image id to identify image.
 * @return true only when validation and ready-bit readback both succeed.
 */
