...
typedef struct
{
    uint8_t ic_type; //!< indicate different ic, RTL8762D is 9
 ...
 * @param  image_id specify the image which located in ota active bank, including app data3-6
 * @return image size of specified image which located in ota active bank
*/
static uint32_t get_active_bank_image_size_by_img_id(T_IMG_ID image_id)
{
    ...
    else
    {
        ...
        else
        {
            if (image_id == OTA)
            {
                image_size = OTA_HEADER_IMAGE_SIZE;
            }
            else
            {
                                                 ...
                                                 T_IMAGE_VERSION *p_image_version)
{
    ...
    {
        image_addr = get_header_addr_by_img_id_imp(image_id);
    }
