...
typedef struct
{
    ...
    bool dfu_conn_para_update_notify_remote;
...
extern bool dfu_service_valid_fw_success(void);
extern bool dfu_service_commit_validated_fw(void);
/* DFU-task-owned inactivity guard; Factory implementations are no-ops. */
extern bool dfu_service_poll_transfer_timeout(void);
extern uint32_t dfu_service_transfer_wait_ms(uint32_t default_wait_ms);
extern void dfu_service_transfer_link_state(uint8_t conn_id, bool connected, uint16_t cause);
