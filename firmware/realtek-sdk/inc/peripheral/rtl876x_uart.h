...
/**
  * Use of the UART_INT_RX_IDLE interrupt requires understanding and acceptance of the following
  * limitations. otherwise it is not recommended.
  * 1. When UART_INT_RX_IDLE is triggered, the customer cannot clear the interrupt flag due to
  *    hardware limitations unless the workflow of resetting rx receiver (0x0020[3] i.e.
  *    REG_STSR->RESET_RCV) before clearing the interrupt flag is followed.Resetting rx receiver
  *    runs the risk of discarding UART receive data.
  * 2. It is recommended that customers use the API rather than directly operating the register
  *    to use UART_INT_RX_IDLE interrupt. Customers can use the UART_INTConfig API to enable or
  *    disable UART_INT_RX_IDLE interrupt which contains the workflow mentioned above.
  */
...
#define IS_UART_GET_INT(INT) ((INT) & (UART_INT_RD_AVA | UART_INT_TX_FIFO_EMPTY | UART_INT_RX_LINE_STS |\
                                       UART_INT_RX_IDLE | UART_INT_TX_DONE | UART_INT_TX_THD ))
...
#define IS_UART_INT_MASK(INT) ((INT) & (UART_INT_MASK_RD_AVA | UART_INT_MASK_TX_FIFO_EMPTY | UART_INT_MASK_RX_LINE_STS |\
                                        UART_INT_MASK_RX_BREAK | UART_INT_MASK_RX_IDLE |\
 ...
 * \note    Use of the UART_INT_RX_IDLE interrupt requires understanding and acceptance of the following
 *          limitations. otherwise it is not recommended.
 *          1. When UART_INT_RX_IDLE is triggered, the customer cannot clear the interrupt flag due to
 *          hardware limitations unless the workflow of resetting rx receiver (0x0020[3] i.e.
 *          REG_STSR->RESET_RCV) before clearing the interrupt flag is followed.
 *          Resetting rx receiver runs the risk of discarding UART receive data.
 *          2. It is recommended that customers use the API rather than directly operating the register
 *          to use UART_INT_RX_IDLE interrupt.
 *          Customers can use the UART_INTConfig API to enable or disable UART_INT_RX_IDLE interrupt
 *          which contains the workflow mentioned above.
 * \param[in] UARTx: UART peripheral selected, x can be 0 ~ 2.
 ...
 * for data length 10 bits(clock source 40MHz only).
 *
