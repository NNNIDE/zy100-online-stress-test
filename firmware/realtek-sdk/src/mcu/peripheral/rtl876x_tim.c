...
#define TIMER_INTTERRUPT_TIME_MIN              (5)
#define TIMER_INTERRUPT_MASK                   BIT(2)
...
/**
  ...
  *         Attention: If interrupt status can't be cleared normally after disable TIM by this API,
  *         you can choose API of TIM_CmdSafe to disable TIM.
...
/**
  * @brief  Protective measures for turning off the TIM.
  *         This function should be used after TIM_ClearINT function otherwise it will occur an error.
  * @param  TIMx: where x can be 0 to 7 to select the TIMx peripheral.
  * @retval None
  */
void TIM_CmdSafe(TIM_TypeDef *TIMx)
{
    uint32_t timerid = 0;
    uint32_t tempreg = 0;
    tempreg = (uint32_t)TIMx;
    timerid = (tempreg - TIM0_REG_BASE) / 20;
    uint32_t status = (TIMER_RAWINTSTATUS & BIT(timerid)) >> timerid;

    /* mask interrupt */
    TIMx->ControlReg |= TIMER_INTERRUPT_MASK;

    /* Check whether the Raw IntStatus is cleared */
    if (status  == 0x1)
    {
        /* Determine whether the current value is less than the min value  */
        if (TIMx->CurrentValue < TIMER_INTTERRUPT_TIME_MIN)
        {
            /* Wait for the Raw IntStatus from 1 to 0 */
            while (((TIMER_RAWINTSTATUS & BIT(timerid)) >> timerid) == 0x1);
